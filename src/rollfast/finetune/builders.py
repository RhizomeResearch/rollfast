...
from rollfast.optim.muon import muon as rollfast_muon
...
def _schedule_config(
    schedule: ScheduleKind | ScheduleConfig,
    total_steps: int | None,
) -> ScheduleConfig:
    if isinstance(schedule, ScheduleConfig):
        return schedule.resolved(total_steps)
    return ScheduleConfig(kind=schedule, total_steps=total_steps)


def _optimizer_axis_name(axis_name: Any) -> str | None:
    return cast(str | None, axis_name)
...
def _multi_transform(
    ...
    return cast(
        ...
        optax.multi_transform(
            cast(Mapping[Hashable, optax.GradientTransformation], transforms),
            lambda _: labels,
        ),
    )
...
def adamw_from_plan(
    ...
    *,
    total_steps: int | None = None,
    base_lr: float = 5e-4,
    schedule: ScheduleKind | ScheduleConfig = "warmup_cosine",
...
def schedule_free_adam_from_plan(
    ...
    *,
    total_steps: int,
    base_lr: float = 5e-4,
    schedule: ScheduleKind | ScheduleConfig = "wsd",
    ...
    *,
    ...
    if family == "aurora":
        return aurora(
            ...
            axis_name=_optimizer_axis_name(axis_name),
            adam_b1=adam_b1,
            adam_b2=adam_b2,
            adam_eps=adam_eps,
            key=key,
...
def _hybrid_method_config(
    ...
    if family == "muon":
        return {
            "method": "muon",
            **config,
            "implemented_profile": "rollfast_muon",
            "reference_algorithm": "muon_newton_schulz_matrix_momentum",
            "matrix_optimizer": "rollfast.optim.muon",
            ...
            "known_deviations": (
                "uses_rollfast_local_profile_not_moonlight_distributed_reference",
...
def _galore_rank_for_group(galore: GaLoreConfig, label: str) -> int:
    if isinstance(galore.rank, Mapping):
        ...
        return int(cast(Mapping[str, int], galore.rank)[label])
